	...
	for {
		...
		go func(uidStr string, originConn net.Conn, wrapped *WrapperedConn) {
			...
			if originalDestination != "" {
				...
				handledConnection, protocol, err := detectTunnelProtocol(originConn)
				if err != nil {
					log.Errorf("detect transparent connection protocol failed: %s", err)
					return
				}
				if protocol == tunnelProtocolUnknown {
				...
				p.handleLoop(protocol == tunnelProtocolTLS, handledConnection, proxyContext)
				return
			}
			...
			if isS5 {
				...
				releaseProxyHandleContext(proxyContext)
				proxyContext = nil
				var protocol tunnelProtocol
				handledConnection, protocol, err = detectTunnelProtocol(handledConnection)
				if err != nil {
				...
				isTls = protocol == tunnelProtocolTLS
				proxyContext, err = CreateProxyHandleContext(subCtx, handledConnection)
				if err != nil {
					log.Error(err)
					return
...
func (p *Proxy) handleConnectionTunnel(req *http.Request, timer *time.Timer, conn net.Conn, ctx *Context, session *Session, brw *bufio.ReadWriter, connectedTo string) error {
	...
	*/
	...
	var protocol tunnelProtocol
	conn, protocol, err = detectTunnelProtocol(conn)
	if err != nil {
		return err
	}
	isTLS := protocol == tunnelProtocolTLS
